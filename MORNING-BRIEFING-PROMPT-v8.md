...
- **Step 3 / Today's Watch List** now explicitly deprioritizes ERCOT audit-timeline items (the December 10 completion deadline, Batch Zero resumption) regardless of how closely they'd otherwise sort to the Top Story — nothing material is expected there before the December PUC meeting, so those items sit near the bottom of the Watch List rather than leading it, until that date approaches.
...
**Same-day follow-up:** the nav in `Layout.astro` was also simplified — the "Morning Briefing" dropdown that listed the 6 most recent dates now links straight to the single newest briefing (still auto-discovered by the existing glob-and-sort logic, just taking the top result instead of the top 6). This required no change to this daily prompt: that logic already ran automatically with no manual nav edits, before or after. Older briefings are no longer linked from the nav at all — they remain fully published, indexed, and permanently reachable through their listing on `trackers.astro`, which is the intended discovery path for anything but the very latest edition.

Everything below (including the Aug 23, 2026 patch notes) is the v7 prompt, with only Step 7, the Step 5 cross-reference, and the CRITICAL CONSTRAINTS pruning exception changed.
...
**STEP 3 — Write the markdown draft.** Match the "Power Grab TX" morning briefing house style — an "⭐ Morning Briefing — [Month D, YYYY]" header (the date, not a fixed tagline — every edition's on-page H1 must be unique per page; a Google crawl in Sept. 2026 flagged all prior editions for sharing one identical H1 across every briefing, which is why the date replaced the old generic "Texas & DFW AI Data Centers, Grid, Land Use" text in the H1 itself — that branding phrase still belongs in the page `<title>`/description, just not in the on-page H1) — but with a bigger structural change from earlier editions: there are no more fixed topic-category headers at all (no more standing "ERCOT & Grid Signals" / "Data Center Construction & Corporate Activity" / etc. section names). The five research areas from Step 2 are scaffolding for gathering the news, not a publishing structure. Instead, present today's edition as a single running list of today's actual stories:
...
* Apply the same "no fixed categories" approach to the closing "Today's Watch List": drop the five institutional subheadings (ERCOT / PUC / TCEQ / County Zoning / Corporate Filings) and instead present it as a single flat bulleted list, ordered by how soon and how directly each item follows from today's stories — the thing most tied to today's Top Story leads. Exception: ERCOT audit-timeline items (the December 10 completion deadline, Batch Zero resumption) are deprioritized regardless of how closely they'd otherwise sort — nothing material is expected there before the December PUC meeting, so keep those items near the bottom of the list rather than letting them lead, until that date approaches.
...
**STEP 7 — Verify old briefing pages are healthy (do not prune).** Old briefing pages are a deliberate long-term asset, not dead weight — treat them like a newspaper's back issues, not clutter. Each one contains dated, specific facts (docket numbers, filings, dollar figures, named parties) that keep earning search traffic long after the day it was published, and every one of them stays permanently linked from `trackers.astro`'s `entries` array (Step 6 never removes an entry), so none of them are ever actually orphaned, even though the nav itself only ever links to the single latest one. Do not delete, prune, blank out, or suggest removing any file in `src/pages/briefings/`, regardless of how many exist.

Instead, use this step as a light health check on the archive:

* List `/Users/development/Development/my-site/src/pages/briefings/` (glob `morning-briefing-*.astro`) now that today's new file has been added, and note the total count.
* Spot-check today's new file — and, if anything looks off, a couple of older ones — for a working self-referencing canonical, no accidental `noindex`, and a real `description` prop (see Step 5's build-error note). This is a lightweight sanity check, not a full audit of every file every day.
* Report the total briefing-page count in your final summary (e.g., "14 briefing pages now published, all retained, no issues found"). This step never produces file deletions or `rm` commands — there is nothing to prune, ever.
...
* Before writing anything to `trackers.astro` or creating a new `briefings/*.astro` file, confirm you have actually completed live research this run — real search and fetch results, not inferred, remembered, or pattern-completed content. If research did not complete for any reason (tool failures, an interrupted run, running out of time), stop and tell Marc directly what happened rather than publish a guess dressed up as news. This applies even under time pressure: a run that cannot finish real research must report itself as unfinished, and must never publish placeholder or invented developments, vote outcomes, or rulings.
...
* Do NOT delete, overwrite, alter, or suggest deleting any existing dated content — past briefing pages, past `scheduled-drafts` files, or past tracker entries. There is no pruning exception: briefing pages, `scheduled-drafts` files, and `trackers.astro` entries are all permanent and are never trimmed, whether by you directly or via a command you hand to the user.
* When done, confirm which files were created/edited, and include the Step 7 archive health-check result (the total briefing-page count and anything flagged in the spot check). Do not attempt to publish, commit, notify externally, delete files yourself, or take any other action.
